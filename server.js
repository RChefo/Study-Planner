...
app.post('/api/auth/google', async (req, res, next) => {
  try {
    ...
    const user = { sub: profile.sub, email: profile.email || '', name: profile.name || profile.email || '', picture: profile.picture || '' };
    ...
    res.cookie(cookieName, token, { httpOnly: true, secure: process.env.NODE_ENV === 'production', sameSite: 'lax', path: '/', maxAge: sessionDays * 86400 * 1000 });
    res.json({ user });
  } catch (err) { next(err); }
});
app.get('/api/auth/me', requireUser, (req, res) => res.json({ user: { sub: req.user.sub, email: req.user.email, name: req.user.name, picture: req.user.picture } }));
app.post('/api/auth/logout', (req, res) => { res.clearCookie(cookieName, { httpOnly: true, secure: process.env.NODE_ENV === 'production', sameSite: 'lax', path: '/' }); res.status(204).end(); });
...
async function main() {
  const required = ['MONGODB_URI', 'GOOGLE_CLIENT_ID', 'SESSION_SECRET'];
