    ...
    children: [
      // Errors inside a page keep the sidebar/top bar and offer a retry.
      {
        ...
        children: [
          ...
          { path: ROUTES.courses, ...page('المواد الدراسية', async () => ({ Component: (await import('@/pages/CoursesPage')).CoursesPage })) },
          { path: `${ROUTES.courses}/:courseId`, ...page('المادة', async () => ({ Component: (await import('@/pages/CourseDetailPage')).CourseDetailPage })) },
          { path: ROUTES.timetable, ...page('جدول المحاضرات', async () => ({ Component: (await import('@/pages/TimetablePage')).TimetablePage })) },
          ...
          { path: ROUTES.timer, ...page('المؤقت', async () => ({ Component: (await import('@/pages/TimerPage')).TimerPage })) },
